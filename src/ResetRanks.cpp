...
class ResetRanksSetting : public SettingV3 {
public:
    ...
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<ResetRanksSetting>();
        auto root = checkJson(json, "ResetRanksSetting");
...
// `SettingBaseValueV3`!
class ResetRanksSettingNode : public SettingNodeV3 {
protected:
    ...
    bool init(std::shared_ptr<ResetRanksSetting> setting, float width) {
        ...
        m_button = CCMenuItemSpriteExtra::create(
            m_buttonSprite, this, menu_selector(ResetRanksSettingNode::onBtn)
        );
    ...
    void onBtn(CCObject* sender) {
		...
		"Reset Ranks?",										 // title
		"Are you sure you want to reset your ranks? This will remove all ranks you have currently.\n\n<cr>Your base save file will NOT be affected.</c>", // content
		"No", "Yes",										 // buttons
		[&](auto, bool btn2)
		{
			if (btn2)
			{
                ...
                for (auto pack : data["main"].as<std::vector<matjson::Value>>().unwrapOr(std::vector<matjson::Value>())) {
                    ...
                    if (saveID != "null") Mod::get()->setSavedValue<matjson::Value>(saveID, {});
                }
                Mod::get()->setSavedValue<std::vector<int>>("completed-levels", std::vector<int>());
				FLAlertLayer::create(
			        "Done!",
			        "Ranks have successfully been reset.",
			        "OK"
		        )->show();
...
public:
    static ResetRanksSettingNode* create(std::shared_ptr<ResetRanksSetting> setting, float width) {
        auto ret = new ResetRanksSettingNode();
    ...
    std::shared_ptr<ResetRanksSetting> getSetting() const {
        return std::static_pointer_cast<ResetRanksSetting>(SettingNodeV3::getSetting());
    }
};

// Define the node creation function
SettingNodeV3* ResetRanksSetting::createNode(float width) {
    return ResetRanksSettingNode::create(
        std::static_pointer_cast<ResetRanksSetting>(shared_from_this()),
        width
    );
}
...
$execute {
    ...
    (void)Mod::get()->registerCustomSettingType("reset-ranks", &ResetRanksSetting::parse);
}
