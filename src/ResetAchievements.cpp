...
class ResetAchievementsSetting : public SettingV3 {
public:
    ...
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<ResetAchievementsSetting>();
        auto root = checkJson(json, "ResetAchievementsSetting");
...
// `SettingBaseValueV3`!
class ResetAchievementsSettingNode : public SettingNodeV3 {
protected:
    ...
    bool init(std::shared_ptr<ResetAchievementsSetting> setting, float width) {
        ...
        m_button = CCMenuItemSpriteExtra::create(
            m_buttonSprite, this, menu_selector(ResetAchievementsSettingNode::onBtn)
        );
    ...
    void onBtn(CCObject* sender) {
		...
		"Reset Achievements?",										 // title
		"Are you sure you want to reset your achievements? This will remove all achievements you have currently.\n\n<cr>Your base save file will NOT be affected.</c>", // content
		"No", "Yes",										 // buttons
		[&](auto, bool btn2)
		{
			if (btn2)
			{
				Mod::get()->setSavedValue<matjson::Value>("achievements", {});
                Mod::get()->setSavedValue<std::vector<int>>("completed-levels", std::vector<int>());
				FLAlertLayer::create(
			        "Done!",
			        "Achevements have successfully been reset.",
			        "OK"
		        )->show();
...
public:
    static ResetAchievementsSettingNode* create(std::shared_ptr<ResetAchievementsSetting> setting, float width) {
        auto ret = new ResetAchievementsSettingNode();
    ...
    std::shared_ptr<ResetAchievementsSetting> getSetting() const {
        return std::static_pointer_cast<ResetAchievementsSetting>(SettingNodeV3::getSetting());
    }
};

// Define the node creation function
SettingNodeV3* ResetAchievementsSetting::createNode(float width) {
    return ResetAchievementsSettingNode::create(
        std::static_pointer_cast<ResetAchievementsSetting>(shared_from_this()),
        width
    );
}
...
$execute {
    ...
    (void)Mod::get()->registerCustomSettingType("reset-achievements", &ResetAchievementsSetting::parse);
}
