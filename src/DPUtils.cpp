...
void DPUtils::updateSaveData() {
	...
	for (std::string index : {"main", "legacy", "bonus", "monthly"}) {
        for (auto pack : data[index].as<std::vector<matjson::Value>>().unwrapOr(std::vector<matjson::Value>())) {
            ...
            auto progress = 0;
            for (auto level : levelIDs) if (DPUtils::containsInt(completedLvls, level)) progress += 1;

            // update status
            auto hasRank = listSave.hasRank || ((progress >= reqLevels) && (reqLevels > -1));
            auto completed = (progress == levelIDs.size());

			if (hasRank != listSave.hasRank || completed != listSave.completed) updateAchievements = true;
	...
	if (updateAchievements && Mod::get()->getSettingValue<bool>("achievement-popups")) DPUtils::updateAchievements();
};

void DPUtils::updateAchievements() {
    auto data = Mod::get()->getSavedValue<matjson::Value>("cached-data");
    ...
    if (!Mod::get()->getSettingValue<bool>("disable-medal-achievements") && data.contains("medals")) {
        ...
        auto progressPercent = StatsPopup::getPercentToRank(data["main"].as<std::vector<matjson::Value>>().unwrapOr(std::vector<matjson::Value>()).size() - 1, true);
