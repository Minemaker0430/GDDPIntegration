...
void DPLayer::openList(CCObject* sender) {
	...
	Mod::get()->setSavedValue<int>("current-pack-totalLvls", levelIDs.size());
	...
	if (type == "main" && Mod::get()->getSavedValue<bool>("is-practice", false)) {
		for (int i = 0; i < levelIDs.size(); i++)
		{
			int num = practiceIDs.at(i).as_int();
			IDs.push_back(std::to_string(num));
		}
	}
	else if (type == "legacy" && Mod::get()->getSavedValue<bool>("is-practice", false)) {
		for (int i = m_data["main"][mainPack]["levelIDs"].as_array().size(); i < practiceIDs.size(); i++)
		{
			int num = practiceIDs.at(i).as_int();
			IDs.push_back(std::to_string(num));
		}
	...
	if (type == "main" || type == "legacy") {
		if (Mod::get()->getSavedValue<bool>("is-practice", false)) {
			packTitle = fmt::format("{} Demons (Practice)", m_data[type][id]["name"].as_string());
		}
		else {
			packTitle = fmt::format("{} Demons", m_data[type][id]["name"].as_string());
		}
	}
	else {
		packTitle = m_data[type][id]["name"].as_string();
	}

	gd::string description = m_data[type][id]["description"].as_string();

	auto list = GJLevelList::create();

	auto glm = GameLevelManager::sharedState();
	glm->m_levelManagerDelegate = this;
	auto searchObject = GJSearchObject::create(SearchType::MapPackOnClick, fmt::format("{}", IDs));
	glm->getOnlineLevels(searchObject);

	log::info("{}", IDs);

	gd::vector<int> levels;

	for (int i = 0; i < IDs.size(); i++)
	{
		int num = stoi(IDs.at(i));
		levels.push_back(num);
	}

	list->m_listName = packTitle;
	list->m_levels = levels;
	list->m_listDesc = ZipUtils::base64URLEncode(description);
