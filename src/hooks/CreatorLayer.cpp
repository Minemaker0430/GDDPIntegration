...
class $modify(CreatorLayer) {
	...
	bool init() {
		...
		if (Mod::get()->getSettingValue<bool>("compact-button")) {
			...
			if (Mod::get()->getSettingValue<bool>("alt-button-texture")) spr = CircleButtonSprite::createWithSpriteFrameName("DP_BeginnerPlus.png"_spr);
			...
			if (Mod::get()->getSettingValue<bool>("alt-button-texture")) spr = CCSprite::createWithSpriteFrameName("DP_demonProgressionBtnAlt.png"_spr);

			if (Loader::get()->isModLoaded("capeling.goodbye_unnecessary_buttons")) spr->setScale(0.85f);
			...
			if (menu->getChildByID("map-packs-button") && Mod::get()->getSettingValue<bool>("replace-map-packs")) {
				dpBtn->setPosition({ menu->getChildByID("map-packs-button")->getPositionX(), menu->getChildByID("map-packs-button")->getPositionY() });

				if (Loader::get()->isModLoaded("alphalaneous.pages_api")) menu->getChildByID("map-packs-button")->removeMeAndCleanup();
				else menu->getChildByID("map-packs-button")->setVisible(false);
			}
			else {
				...
				if (Loader::get()->isModLoaded("cvolton.betterinfo") && !Mod::get()->getSettingValue<bool>("dont-move-side-btns")) {
					if (auto menu = this->getChildByID("cvolton.betterinfo/center-right-menu")) menu->setPositionY(260);
				}

				if (Loader::get()->isModLoaded("spaghettdev.gd-roulette") && !Mod::get()->getSettingValue<bool>("dont-move-side-btns")) {
					if (auto menu = this->getChildByID("spaghettdev.gd-roulette/creator-layer-menu")) menu->setPositionY(92);
					else if (auto menu = this->getChildByID("spaghettdev.gd-roulette/center-left-menu")) menu->setPositionY(255);
				}
			}
		}
